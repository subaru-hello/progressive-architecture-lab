...
export async function listLatest100(readPool: Pool): Promise<Pick<Item, 'id' | 'name' | 'created_at'>[]> {
...
export async function getById(pool: Pool, id: number): Promise<Item | null> {
...
export async function create(pool: Pool, name: string): Promise<Pick<Item, 'id' | 'name' | 'created_at'>> {
  const { rows } = await pool.query(
...
export async function decrementStock(pool: Pool, id: number, qty: number): Promise<{ ok: boolean; stock: number }> {
  const { rows } = await pool.query(
