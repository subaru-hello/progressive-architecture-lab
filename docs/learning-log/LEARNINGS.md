...
最終更新: 2026-07-13（Lv19 — 分散tx(none/2PC/saga)。引っ越し軸 Lv16-19 完了）
...
## 35. ⭐ 「引っ越しやすさ」は monolith か micro かでなく、境界の結合度で決まる — 泥団子は live で動かせない `[Lv16]`
分解の「引っ越し軸」の出発点。故意に絡ませた**本物の泥団子**(`ARCH=mud`: `POST /orders` が items+orders+users を
1 tx で横断・`GET /orders` が cross-domain JOIN・orders→items/users の跨ぎ FK)を作り、items のライブ抽出を試みた。
**mud は遅くも壊れてもいない**(baseline order med 5.85ms≈hex Lv13・error 0%)——**ただ動かせない**: 跨ぎ FK が
`DROP TABLE items` を物理的に拒否し(`constraint orders_item_id_fkey depends on it`)、直 SQL/JOIN が items 不在で即死、
**naive 引っ越し中は 100% ダウン**(843/843 が 5xx)。結合(共有 tx 1・JOIN 1・跨ぎ FK 2)の一つ一つが「新 DB を跨げない
縫い目」で、これを数える＝migratability を測ること。**monolith だから移せないのではなく境界が絡んでいるから移せない**。
綺麗にして「切る場所に seam を仕込む」のが移行の本体（→ Lv17 で結合を hexagonal に解きほぐし seam 1 本に落とす、
Lv18 で同じ引っ越しがダウンタイムゼロで通ることを Lv16 の 100% ダウンと対比）。教訓8「回避でなく根治」の移行版。

## 36. ⭐ 綺麗にする＝「切る場所に seam を仕込む」こと。hexagonal 境界＝事前に引いた移行の切断線 `[Lv17]`
Lv16 の泥団子を DB/Domain/Usecase/Infra の ports & adapters に解きほぐした。**結合度が seam 1 本に激減**:
cross-domain JOIN 1→0・跨ぎ FK 2→0・3テーブル共有 tx 1→0、orders の他ドメイン依存は **ItemsPort/UsersPort
interface ただ 1 本**に（port モードでは他ドメインの表に一切触れず、join の UPDATE items すら RepoPort 裏の最適化で
env 1 個で port に切替可・usecase/domain は無変更）。**これがそのまま抽出の seam**。**層化は runtime タダ**
(order med 4.85ms ≈ mud/Lv13、interface 越しの間接呼び出しは無料＝Lv14「modular は runtime 無料」の再演)——
払うのは設計規律であって性能ではない。2つの「cargo-cult しない」判断: ①**usecase 層はオーケストレーションがある
...
## 38. ⭐⭐ タダの分散tx は無い：2PC は原子性(C)をブロッキングで買い、saga は可用性(A)を中間状態の可視化で買う `[Lv19]`
Lv18 で items が別DBになった後、POST /orders の在庫(items-db)+order(orders-db)を2DB跨ぎで調停する3方式を障害注入で実測:
- **none**(med 3.42ms・最速): 無防備。障害で「在庫だけ -20・order 0」の**恒久リーク**(教訓34 の放置版)。
- **2pc**(med 5.46ms・最遅, 本物の PG `PREPARE TRANSACTION`/`COMMIT PREPARED`): happy は all-or-nothing だが 2 相×往復×fsync で遅い。
  **coordinator が両 prepare 後に死ぬと両DBに prepared txn が宙吊り(in-doubt)・行ロックを握り手動 `ROLLBACK PREPARED` まで
  後続をブロック**。→ **原子性(C)を可用性(A)で買う**。
- **saga**(med 5.04ms, orchestration+冪等 reservation+補償+recovery poller): ロックを一切握らず、クラッシュしても poller が
  ~6s で補償し結果整合に収束。だが「予約済だが未 confirm」の**中間状態が可視**・補償/冪等/outbox を自前実装。→ **可用性(A)を分離性(I)で買う**。
- **= CAP/PACELC がアプリ層に降りる**。多くの現代システムが saga を選ぶのは A とスケールを優先し I を諦める判断。
- **2PC の脆さを実地で踏んだ**: commit の HTTP が「空 body + application/json」で Fastify に 400→prepared が commit されず
  orphan 化→行ロックが溜まり後続 timeout→**orphan 雪だるまで全滅**。「commit phase の 1 失敗が in-doubt を生み伝播する」
  2PC の本質的脆さそのもの。加えて高競合(RATE=30)では prepared ロック衝突で prepare が timeout し `max_prepared_transactions`
  を食い潰す——**2PC はスループットと相性が悪い**。教訓34「原子性が自前の分散tx問題に化ける」の回収編。
...
## 41. ⭐ exactly-once「配送」は無い。exactly-once「効果」＝ at-least-once ＋ 冪等消費者 — outbox は原子性を分散txから単一DB tx+dedup へ格下げする `[Lv22]`
`none` の dual-write(decrement→order insert の別々2書込)は障害で lost(stock 減・order 無)/ghost(order 有・decrement 無)。
transactional outbox で潰した(app のみ・外部契約不変):
- **原子的 emit**: order = 同一 orders-db tx で orders INSERT + outbox INSERT(msg_id)。dual-write 窓が消える。
- **at-least-once 配送**: poller が未配送 outbox を items receiver へ HTTP 配送・成功で mark。クラッシュしても未配送は残り再送。
- **冪等 receiver**: items 側で同一tx `INSERT processed_messages ON CONFLICT DO NOTHING` → 新規なら stock 更新・既処理なら no-op。
  dedup と stock 更新が同一 tx なので中間状態が無い。
- **実測**: happy=配送で stock 1回減。fault(after-deliver-before-mark)で poller が**3回再配送しても stock は 5 だけ**(dedup)、
  再起動で stuck message は再配送→dedup→mark に収束・stock 不変。
- **学び**: ①「exactly-once 配送」は 2 generals で原理的に無い。有るのは **at-least-once × 冪等消費者 = exactly-once 効果**。
  outbox は冪等キー(msg_id)を送り手のローカル tx で採番・永続化する仕掛け。②**2PC(教訓38-40)が原子性をブロッキング
  (prepared ロック/in-doubt/coordinator 回復)で買うのに対し、outbox はロックも in-doubt も持たず poller+dedup だけで買う**——
  現代システムが emit に 2PC でなく outbox を選ぶ理由。③代償は同期の確実さ→非同期の結果整合(order は stock 確定前に 201・
  同期在庫チェックを落とす=負在庫あり得る)。壁を消すのでなく位置を替える取引(教訓33 の再演)。
