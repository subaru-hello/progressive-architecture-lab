...
最小構成                水平スケールの初歩         宣言的デプロイ            自動スケール・耐障害
  ...
  引っ越しやすさ＝結合度      結合を seam 1本に(JOIN/FK→0)  規律=0%/naive=66.7%/泥団子=100%   cross-DB原子性を saga/2PCで買う
  naive移行は100%ダウン       層化は runtime 無料(4.85ms)  seam×手順の積・shadowで検証後cutover (タダの分散txは無い)
```
